...
//********************************************************************
// kill_wizards - cancels all the wizard threads once a winner
//					has been declared
//	pthread_t *wid - the array of wizard threads
//******************************************************************** 
void 
kill_wizards(pthread_t *wid)
{
	int i, j;
...
//********************************************************************
// check_winner - checks to see if an entire team has been frozen
// 	struct cube *cube - takes in the entire cube struct
//********************************************************************
bool 
...
main(int argc, char** argv)
{
	...
	/* User and Wizard sem initialization */
	sem_init(&user, 0, 1);
	sem_init(&wiz, 0, 0);
...
fight_wizard(struct wizard *self, struct wizard *other, struct room *room)
{
	...
	if (res == 0)
    {
		...
		/* Sets other wizard status and waits on semaphore */
		other->status = 1;
		sem_wait(&other->frz);
    }
	...
	else
    {
		...
		/* Sets self wizard status and waits on semaphore */
		self->status = 1;
		sem_wait(&self->frz);
    }	
	return 0;
...
free_wizard(struct wizard *self, struct wizard *other, struct room* room)
{
	...
	if (res == 0)
    {
		...
		/* Sets other wizard status and posts on semaphore */
		other->status = 0;
		sem_post(&other->frz);
		return 0;
    }
